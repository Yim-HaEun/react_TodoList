...
const App = () => {
  const savedTodos = JSON.parse(localStorage.getItem('posts')) || [];
  //위의 const App() =>{ 은 function App() {과 같음  //todos는 할일 목록들을 저장하는 공간 / newTodo는 새로운 할 일을 추가 작성할 수 있는 공간
  const [todos, setTodos] = useState(savedTodos); //todos 초기값을 빈 배열로 생성하겠다는 의미
            ...
            <li id="size" key={index}>
              {editingIndex === index ? (
                <div>
                  ...
                  <button id="edit" onClick={saveEdit}>
                    저장
                  </button>
                  <button id="delete" onClick={cancelEdit}>
                    취소
                  </button>
                </div>
              ) : (
                <div id="text">
                  {todo}
                  <button id="edit" onClick={() => startEditing(index, todo)}>
                    edit
                  </button>
